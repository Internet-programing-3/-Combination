...
<body>
	...
	String bookCtg = ""; // bookCtg 변수 선언
        ...
        try {
            ...
            // 베스트셀러 목록이 있다면 첫 번째 행의 bookCtg 값을 가져와 설정
            if (bestSellerRs.next()) {
                bookCtg = bestSellerRs.getString("bookCtg");
            }
	%>
	<!-- 제이쿼리용 -->
...
<nav class="navMainOne">
	...
	<main class="ListMain">
		...
		<section>
			...
			<%
			// 베스트셀러 목록을 가져오는 부분에서 bookCtg 값을 설정하는 코드를 루프 밖으로 이동
            bestSellerRs.close();
            bestSellerStmt.close();

            // 다시 베스트셀러 목록을 가져오기 위해 쿼리 실행
            bestSellerStmt = con.prepareStatement(bestSellerQuery);
            bestSellerRs = bestSellerStmt.executeQuery();

            int bookId = 0; // 기본값으로 초기화
            String bookImg = "";
            String bookName = "";
            String writer = "";
            String publisher = "";
            ...
            while (bestSellerRs.next()) {
                bookId = bestSellerRs.getInt("bookId");
                bookName = bestSellerRs.getString("bookName");
                price = bestSellerRs.getInt("price");
                writer = bestSellerRs.getString("writer");
                publisher = bestSellerRs.getString("publisher");
                bookCtg = bestSellerRs.getString("bookCtg");
                bookStatus = bestSellerRs.getString("bookStatus");
                bookContent = bestSellerRs.getString("bookContent");
                bookStock = bestSellerRs.getInt("bookStock");
                bookReview = bestSellerRs.getString("bookReview");
                bookImg = bestSellerRs.getString("bookImg");
